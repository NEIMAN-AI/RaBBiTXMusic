...
from Rabbit import BUNNY_MENTION, LOGGER, SUDOERS, app, app2
    ...
    if message.reply_to_message.photo:
        ...
        try:
            ...
            return await fuk.edit_text(
                f"» {BUNNY_MENTION} ᴘʀᴏғɪʟᴇ ᴘɪᴄ ᴄʜᴀɴɢᴇᴅ ʙʏ {BOT_NAME} sᴜᴄᴄᴇssғᴜʟʟʏ."
            )
        except:
...
async def set_bio(_, message: Message):
    ...
    if msg:
        if msg.text:
            ...
            return await message.reply_text(
                f"» {BUNNY_MENTION} ʙɪᴏ ᴄʜᴀɴɢᴇᴅ ʙʏ {BOT_NAME} sᴜᴄᴄᴇssғᴜʟʟʏ."
            )
    elif len(message.command) != 1:
        newbio = message.text.split(None, 1)[1]
        await app2.update_profile(bio=newbio)
        return await message.reply_text(f"» {BUNNY_MENTION} ʙɪᴏ ᴄʜᴀɴɢᴇᴅ ʙʏ {BOT_NAME} sᴜᴄᴄᴇssғᴜʟʟʏ.")
    else:
        return await message.reply_text(
...
async def set_name(_, message: Message):
    ...
    if msg:
        if msg.text:
            ...
            return await message.reply_text(
                f"» {BUNNY_MENTION} ɴᴀᴍᴇ ᴄʜᴀɴɢᴇᴅ ʙʏ {BOT_NAME} sᴜᴄᴄᴇssғᴜʟʟʏ."
            )
    elif len(message.command) != 1:
        name = message.text.split(None, 1)[1]
        await app2.update_profile(first_name=name, last_name="")
        return await message.reply_text(f"» {BUNNY_MENTION} ɴᴀᴍᴇ ᴄʜᴀɴɢᴇᴅ ʙʏ {BOT_NAME} sᴜᴄᴄᴇssғᴜʟʟʏ.")
    else:
        return await message.reply_text(
